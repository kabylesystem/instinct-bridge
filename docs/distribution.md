...
2. **Public GitHub preview:** attach the archive and SHA-256 checksums directly to the release, so users do not need to find a CI artifact or install Python. Keep the code, install instructions, exact supported scope and synthetic evidence in the repository.
...
No Vercel deployment is needed for the importer.
