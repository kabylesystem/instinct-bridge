...
## Current implementation — Preview 0.6.0
...
51 tests cover parsing, crypto, web authorization, capture boundaries, transfer progress, uncertain failures and cleanup. The one-command source launcher and in-app flow prioritize Bitwarden login migration. Authy setup is optional; the review keeps the transfer action above a folded, searchable account list, shows selection totals, and displays progress during transfer. The UI accepts duplicate Bitwarden titles, adds the site hostname and a readable username hint to each new target label, indicates password presence without exposing values, and uses source item IDs to skip identical records on later exports. Existing labels are not rewritten on reimport. Conflicting changes are not overwritten. Real Instinct synthetic transfer/read-back/repeat/conflict/cleanup pass, including same-service multi-account and empty-field records. A complete real login batch was transferred with per-item read-back and a fresh-session inventory check. See docs/evidence for synthetic test reports; no private credentials appear there. USB pairing is validated, but Authy storage is inaccessible and actual phone extraction remains pending. Other Bitwarden item types, other authenticators and cross-platform runtime support are not delivered.

The 0.6.0 interface removes repeated copy, reveals the Bitwarden password field only for encrypted exports, and keeps extra-field details folded beneath the primary transfer action. The chosen distribution is a local executable with a browser UI; [the distribution decision](distribution.md) explains why a hosted Vercel importer is not the current path.
